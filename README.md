...
Execution Flow
User provides one or more destination hosts.
The program resolves each hostname to an IP address.
ICMP Echo Requests are sent using raw sockets.
RTT is measured for each reply received.
Packet loss statistics are calculated.
TTL values are incremented to perform traceroute and discover network hops.
Results are printed for each destination.
